...
const List = () => {
    ...
    const getCategorys = async () => {
        try {
            ...
            const response = await ApiService.request({
                method: "GET",
                url: "categories", // Replace with your API endpoint
                data: apiData
            });
    ...
    const delData = async (id) => {
        ...
        if (result.isConfirmed) {
            const response = await ApiService.request({
                method: "DELETE",
                url: `categories/${id}`, // Replace with your API endpoint
            });
            const data = response.data;
            if (data.status) {
                getCategorys();
    ...
    const updateStatus = async (id) => {
        ...
        if (result.isConfirmed) {
            const response = await ApiService.request({
                method: "POST",
                url: `categories/status/${id}`, // Replace with your API endpoint
            });
            const data = response.data;
            if (data.status) {
                getCategorys();
        ...
        <div className="d-flex flex-column flex-column-fluid">
            <div id="kt_app_toolbar" className="app-toolbar py-3 py-lg-6">
                <div id="kt_app_toolbar_container" className="app-container container-xxl d-flex flex-stack">
                    ...
                    <div className="d-flex align-categories-center gap-2 gap-lg-3">
                        {/* Add New categories Button */}
                        <Link to={`/categories/create`}  className='btn btn-base btn-sm'>{("Add New Category")}</Link>
                    </div>
                </div>
            </div>
                ...
                <div id="kt_app_content_container" className="app-container container-xxl">
                    <div className="card">
                                ...
                                <form method="get">
                                    <div className="row">
                                        <div className="col-md-4">
                                            <div className="d-flex align-categories-center position-relative my-1">
                        ...
                        <div className="card-body pt-0">
                            <div id="kt_categories_table_wrapper" className="dataTables_wrapper dt-bootstrap4 no-footer">
                                        ...
                                        <tbody className="fw-semibold text-gray-600 fs-6">
                                            {categories.length > 0 ? categories.map((cat, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className="mt-2">
                                                        <Link to={`/categories/${cat._id}`} className="btn btn-sm h-35px btn-light-primary">
                                                            Sub Categories
                                                        </Link>
                                                        <Link to={`/categories/create/${cat._id}`} className="btn btn-sm h-35px btn-light-primary">
