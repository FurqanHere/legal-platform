...
const List = () => {
    ...
    useEffect(() => {
        getBanners();
    }, [apiData.page, apiData.search, apiData.daterange,tab]); // Dependency array for re-fetching data
    ...
    const getBanners = async () => {
        try {
            ...
            const response = await ApiService.request({
                method: "GET",
                url: "banners", // Replace with your API endpoint
                data: apiData
            });
    ...
    const updateStatus = async (id) => {
        ...
        if (result.isConfirmed) {
            const response = await ApiService.request({
                method: "POST",
                url: `banners/status/${id}`, // Replace with your API endpoint
            });
            const data = response.data;
            if (data.status) {
                getBanners();
        ...
        <div className="d-flex flex-column flex-column-fluid">
            <div id="kt_app_toolbar" className="app-toolbar py-3 py-lg-6">
                <div id="kt_app_toolbar_container" className="app-container container-xxl d-flex flex-stack">
                    ...
                    <div className="d-flex align-banners-center gap-2 gap-lg-3">
                        {/* Add New banners Button */}
                        <Link to={`/banners/create`} className='btn btn-base btn-sm'>{("Add New Banner")}</Link>
                    </div>
                </div>
            </div>
                    ...
                    <div className="card">
                        <nav>
                            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                <button className={`nav-link fs-lg-5 ${tab === "home" ? "active" : ""}`}
                                    onClick={() => {
                                        setTab("home");
                                        setApiData(prevData => ({
                                          ...prevData,
                                      ...
                                      }} id="nav-profile-tab" type="button"
                                    ><i class="bi bi-house-door-fill"></i> Home Banners
                                </button>
                                <button className={`nav-link fs-lg-5 ${tab === "promotional" ? "active" : ""}`}
                                    onClick={() => {
                                        setTab("promotional");
                                        setApiData(prevData => ({
                                          ...prevData,
                                      ...
                                      }} id="nav-contact-tab" type="button"><i class="bi bi-file-image-fill"></i> 
                                   &nbsp; Promotional Banners
                                </button>
                            </div>
                        </nav>
                                ...
                                <form method="get">
                                    <div className="row">
                                        <div className="col-md-4">
                                            <div className="d-flex align-banners-center position-relative my-1">
                        ...
                        <div className="card-body pt-0">
                            <div id="kt_banners_table_wrapper" className="dataTables_wrapper dt-bootstrap4 no-footer">
                                <div className="table-responsive">
                                    <table className="table align-middle table-hover fs-6 gy-5 dataTable no-footer">
                                        ...
                                        <tbody className="fw-semibold text-gray-600 fs-6">
                                            {banners.length > 0 ? banners.map((cat, index) => (
                                                <tr key={index}>
                                                    <td>
                                                    ...
                                                    <td className="mt-2">
                                                        <Link to={`/banners/create/${cat._id}`} className="btn btn-sm h-30px btn-light-primary">
