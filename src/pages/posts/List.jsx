...
const List = () => {
  ...
  useEffect(() => {
    getPost();
  }, [apiData.page, apiData.search, apiData.daterange, apiData.type]); 
  ...
  const delData = async (id) => {
    ...
    if (result.isConfirmed) {
      const response = await ApiService.request({
        method: "DELETE",
        url: `posts/${id}`, // Replace with your API endpoint
      });
      const data = response.data;
      if (data.status) {
        getPost();
  ...
  const updateStatus = async (id) => {
    ...
    if (result.isConfirmed) {
      const response = await ApiService.request({
        method: "POST",
        url: `companies/status/${id}`, // Replace with your API endpoint
      });
      const data = response.data;
      if (data.status) {
        getPost();
        ...
        >
          <div className="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            ...
            <ul className="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
              ...
              <li className="breadcrumb-item text-muted">Posts</li>
            </ul>
          </div>
        </div>
      </div>
                ...
                <form method="get">
                  <div className="row">
                    <div className="col-md-4">
                      <div className="d-flex align-companies-center position-relative my-1">
                        <input
                          type="text"
                      ...
                      {companies.length > 0 ? (
                        companies.map((cat, index) => (
                          <tr key={index}>
                            ...
                            <td>
                              <Link to={`/posts/${cat._id}`}>
                                <img src={cat?.company_id?.logo} className="w-45px h-45px rounded-pill me-2" alt="" />
                                {cat?.company_id?.company_name}
                              </Link>
                            </td>
                            <td>
                              <Link to={`/posts/${cat._id}`}>{cat.title}</Link>
                            </td>
                            <td>
                              <Link to={`/posts/${cat._id}`}>
                                <span
                            ...
                            <td className="mt-2">
                              ...
                              <Link
                                to={`/posts/create/${cat._id}`}
                                className="btn btn-sm h-30px btn-light-primary"
                              >
                                <i className="bi bi-pencil-fill fs-6"></i>
                              </Link>
                              <button
                                onClick={() => delData(cat._id)}
                                className="btn btn-sm h-30px btn-light-primary"
                              >
