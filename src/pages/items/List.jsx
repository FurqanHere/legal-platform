...
const List = () => {
    ...
    const getItems = async () => {
        try {
            ...
            const response = await ApiService.request({
                method: "GET",
                url: "items", // Replace with your API endpoint
                data: apiData
            });
    ...
    const delData = async (id) => {
        ...
        if (result.isConfirmed) {
            const response = await ApiService.request({
                method: "DELETE",
                url: `items/${id}`, // Replace with your API endpoint
            });
            const data = response.data;
            if (data.status) {
                getItems();
    ...
    const updateStatus = async (id) => {
        ...
        if (result.isConfirmed) {
            const response = await ApiService.request({
                method: "POST",
                url: `items/status/${id}`, // Replace with your API endpoint
            });
            const data = response.data;
            if (data.status) {
                getItems();
            ...
            <div id="kt_app_toolbar" className="app-toolbar py-3 py-lg-6">
                <div id="kt_app_toolbar_container" className="app-container container-xxl d-flex flex-stack">
                    ...
                    <div className="d-flex align-items-center gap-2 gap-lg-3">
                        ...
                        <Link to={`/items/create`}  className='btn btn-base btn-sm'>Add New Item</Link>
                    </div>
                </div>
            </div>
                ...
                <div id="kt_app_content_container" className="app-container container-xxl">
                    <div className="card">
                        ...
                        <div className="card-body pt-0">
                            <div id="kt_items_table_wrapper" className="dataTables_wrapper dt-bootstrap4 no-footer">
                                <div className="table-responsive">
                                    <table className="table align-middle table-hover fs-6 gy-5 dataTable no-footer">
                                        ...
                                        <tbody className="fw-semibold text-gray-600 fs-6">
                                            {items.length > 0 ? items.map((cat, index) => (
                                                <tr key={index}>
                                                    <td>
                                                        {cat.image && 
                                                            <img src={cat.image} alt="image" className="w-50px h-50px object-cover rounded" />
                                                        }
                                                    </td>
                                                    <td>{cat.title}</td>
                                                    <td> 
                                                        {cat.discounted_price ?(
                                                            <div>
                                                                <strike className="text-danger">{currency}  {cat.original_price}</strike> <br />
                                                                <span>{currency}  {cat.discounted_price}</span>
                                                            </div>
                                                        ):(
                                                            <span>{currency}  {cat.original_price}</span>
                                                        )}
                                                    ...
                                                    <td className="mt-2">
                                                        <Link to={`/items/${cat._id}`} className="btn btn-sm h-30px btn-light-primary">
                                                            <i className="bi bi-eye-fill fs-6"></i>
                                                        </Link>
                                                        <Link to={`/items/create/${cat._id}`} className="btn btn-sm h-30px btn-light-primary">
